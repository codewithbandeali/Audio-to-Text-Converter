...
if st.button("Convert to Audio"):
    ...
    # Merge the audio files into a single file
    concatenated_audio_path = os.path.join(output_folder, "output.mp3")
    concatenation_command = f"ffmpeg -i \"concat:{'|'.join([f'{output_folder}/chunk_{i+1}.mp3' for i in range(len(text_chunks))])}\" -acodec copy {concatenated_audio_path}"
    ...
    if os.path.exists(concatenated_audio_path):
        ...
        st.audio(audio_bytes, format='audio/mp3', key='audio')
        
        # Provide download link
        st.button("Download Audio", key='download_button',
                  on_click=lambda: download_audio(concatenated_audio_path, "output.mp3"))
    else:
        st.error(f"Audio file '{concatenated_audio_path}' not found.")

# Function to download audio file
def download_audio(file_path, file_name):
    with open(file_path, "rb") as audio_file:
        audio_bytes = audio_file.read()
        b64 = base64.b64encode(audio_bytes).decode()
        href = f'<a href="data:audio/mp3;base64,{b64}" download="{file_name}">Click here to download</a>'
